...
int handlerLiveData(UrlHandlerParam* param)
{
    ...
    n += obdUserPidsAppendLiveJson(buf + n, bufsize - n, t);
    n += snprintf(buf + n, bufsize - n, "}");
#if USE_MEMS
...
void setup()
{
    ...
    if (g_rt_ble) {
      ...
      ble_init(bleNm);
      Serial.println("[BLE] advertising (see System tab to disable; reboot to apply changes)");
    } else {
...
#if ENABLE_HTTPD && ENABLE_WIFI && ENABLE_WIFI_PORTAL
                api_push_add_sample((uint16_t)pid | 0x100, (double)obdData[i].value);
#endif
            } else {
                pidErrors++;
                Serial.print("PID ");
...
#if ENABLE_HTTPD && ENABLE_WIFI && ENABLE_WIFI_PORTAL
        api_push_add_sample((uint16_t)PID_BATTERY_VOLTAGE, (double)batteryVoltage);
#endif
