...
void CBuffer::forEachNumeric(NumericVisitor vis, void* user)
{
  uint16_t of = 0;
  for (int n = 0; n < total && of < offset; n++) {
    ELEMENT_HEAD* hdr = (ELEMENT_HEAD*)(m_data + of);
    ...
    switch (hdr->type) {
    case ELEMENT_UINT8: {
      const uint8_t* p = (const uint8_t*)(m_data + of);
      double s = 0;
      for (int i = 0; i < c; i++) s += p[i];
      of += c;
      if (vis) vis(hdr->pid, s / (double)c, user);
    } break;
    case ELEMENT_UINT16: {
      const uint16_t* p = (const uint16_t*)(m_data + of);
      double s = 0;
      for (int i = 0; i < c; i++) s += p[i];
      of += c * 2;
      if (vis) vis(hdr->pid, s / (double)c, user);
    ...
    case ELEMENT_UINT32: {
      const uint32_t* p = (const uint32_t*)(m_data + of);
      double s = 0;
      for (int i = 0; i < c; i++) s += (double)p[i];
      of += c * 4;
      if (vis) vis(hdr->pid, s / (double)c, user);
    } break;
    case ELEMENT_INT32: {
      const int32_t* p = (const int32_t*)(m_data + of);
      double s = 0;
      for (int i = 0; i < c; i++) s += (double)p[i];
      of += c * 4;
      if (vis) vis(hdr->pid, s / (double)c, user);
    ...
    case ELEMENT_FLOAT_D1:
    case ELEMENT_FLOAT_D2: {
      const float* p = (const float*)(m_data + of);
      double s = 0;
      for (int i = 0; i < c; i++) s += (double)p[i];
      of += c * 4;
      if (vis) vis(hdr->pid, s / (double)c, user);
...
void CBufferManager::printStats()
{
  ...
  if (slots) {
    serialLogPrintf("[BUF] %d samples | %d bytes | %d/%d\n", samples, bytes, count, total);
  }
}
...
bool TeleClientUDP::notify(byte event, const char* payload)
{
  ...
  for (byte attempts = 0; attempts < 3; attempts++) {
    ...
    if (!verifyChecksum(data)) {
      serialLogPrintf("[UDP] Checksum mismatch:%s\n", data);
      continue;
    }
    char pattern[16];
    sprintf(pattern, "EV=%u", event);
    if (!strstr(data, pattern)) {
      serialLogPrintf("[UDP] Invalid reply: %s\n", data);
      continue;
    }
...
bool TeleClientUDP::transmit(const char* packetBuffer, unsigned int packetSize)
{
  ...
  if (wifi.connected()) {
    if (wifi.send(packetBuffer, packetSize)) {
      txBytes += packetSize;
      txCount++;
      serialLogPrintf("[WIFI] %u bytes sent\n", (unsigned)packetSize);
      return true;  
    }
    return false;
  }
  ...
  serialLogPrintf("[CELL] %u bytes being sent\n", (unsigned)packetSize);
