...
#if ENABLE_WIFI && ENABLE_WIFI_PORTAL
    {"favicon.ico", handlerFavicon},
    {"portal.js", handlerPortalJs},
    {"", handlerPortalHtml},
    {"portal", handlerPortalHtml},
    {"api/portal/login", handlerPortalLogin},
    {"api/portal/scan", handlerPortalScan},
    {"api/portal/connect", handlerPortalConnect},
    {"api/portal/status", handlerPortalStatus},
    {"api/portal/obd-pids", handlerPortalObdPids},
    {"api/portal/remote-test", handlerPortalRemoteTest},
    {"api/portal/remote", handlerPortalRemote},
    {"api/portal/bt", handlerPortalBt},
    {"api/portal/cfg", handlerPortalCfg},
#endif
...
bool serverSetup(IPAddress& ip)
{
    ...
    {
      int want_ap = ENABLE_WIFI_AP ? (int)g_rt_wifi_ap : 0;
      int want_sta = ENABLE_WIFI_STATION ? (int)g_rt_wifi_sta : 0;
      if (want_ap && want_sta) {
        ...
        Serial.println("[NET] WiFi off in NVS — set AP or STA in portal System, then reboot");
        return false;
      }
    }
#if ENABLE_WIFI_AP
...
#endif
    ip = g_rt_wifi_ap ? WiFi.softAPIP() : WiFi.localIP();
    if (g_rt_wifi_ap) {
    Serial.print("[HTTPD] AP \"");
    Serial.print(WIFI_AP_SSID);
    Serial.print("\" open — portal http://");
    Serial.print(WiFi.softAPIP());
    Serial.println("/ (or /portal)");
    } else {
    Serial.print("[HTTPD] softAP off; portal if STA: http://");
    Serial.print(ip);
    Serial.println("/ (or /portal)");
    }
#else
...
#if NET_DEVICE == NET_WIFI
    /* SNTP needs a routed STA path; starting it on AP-only config has tripped asserts on some builds. */
